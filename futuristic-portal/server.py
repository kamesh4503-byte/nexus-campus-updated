...
def _merge_defaults(current, default):
    """Recursively fill missing keys without overwriting valid admin data."""
    if isinstance(default, dict):
        if not isinstance(current, dict):
            current = {}
        for key, value in default.items():
            current[key] = _merge_defaults(current.get(key), value) if key in current else json.loads(json.dumps(value))
        return current
    return current if current is not None else json.loads(json.dumps(default))
    ...
    with DATA_LOCK:
        ...
        try:
            ...
            if not isinstance(raw, dict):
                raise ValueError("portal data root must be an object")
        except Exception:
            raw = deep_copy_default()
            recovered = True

        data = _merge_defaults(raw, DEFAULT_DATA)
        # Structural sanity for timetable, because malformed JSON should never break every page.
        tt = data.get("timetable", {})
        days, periods, grid = tt.get("days", []), tt.get("periods", []), tt.get("grid", [])
        if not isinstance(days, list) or not isinstance(periods, list) or not isinstance(grid, list) or len(grid) != len(days) or any(not isinstance(row, list) or len(row) != len(periods) for row in grid):
            data["timetable"] = json.loads(json.dumps(DEFAULT_DATA["timetable"]))
            recovered = True

        for key in ("assignments", "announcements", "events"):
            if not isinstance(data.get(key), list):
                data[key] = json.loads(json.dumps(DEFAULT_DATA[key]))
                recovered = True
        if not isinstance(data.get("attendance"), dict):
            data["attendance"] = json.loads(json.dumps(DEFAULT_DATA["attendance"]))
            recovered = True
        if not isinstance(data.get("profile"), dict):
            data["profile"] = json.loads(json.dumps(DEFAULT_DATA["profile"]))
            recovered = True

        # Always write a clean recovered file so a corrupted file doesn't fail on every request.
        if recovered:
            temp = DATA_FILE + ".tmp"
...
class Handler(http.server.SimpleHTTPRequestHandler):
    def end_headers(self):
        # Prevent stale HTML/CSS/JS after Render redeploys. Version query strings are kept too,
        # but these headers make updates deterministic even if a browser/proxy cached an old file.
        parsed = urllib.parse.urlparse(self.path)
        if not parsed.path.startswith("/api/"):
            ext = os.path.splitext(parsed.path)[1].lower()
            if parsed.path in ("", "/") or ext in (".html", ".css", ".js", ".json"):
    ...
    def _admin_ok(self, payload=None, query=None):
        payload = payload or {}
        query = query or {}
        supplied = self.headers.get("X-Admin-Password", "") or payload.get("password") or (query.get("password") or [""])[0]
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            return self._send_json({"status": "ok", "service": "nexus-campus"})
    ...
    def do_POST(self):
        ...
        if path == "/api/admin/upload-pdf":
            ...
            if not content.startswith(b"%PDF"):
                return self._send_json({"error": "That file does not appear to be a valid PDF"}, 400)
        ...
        if match:
            submission = str(payload.get("text", "")).strip()[:4000]
            if not submission:
                return self._send_json({"error": "Submission text is required"}, 400)
            data = load_data()
            aid = match.group(1)
            found = False
            for a in data["assignments"]:
        ...
        if path.startswith("/api/admin/"):
            ...
            if path == "/api/admin/event":
                ...
                if item["type"] not in ("event", "exam", "deadline"):
                    item["type"] = "event"
                try:
            ...
            if path == "/api/admin/timetable":
                try:
                    ...
                    if not subject:
                        raise ValueError
                    days = data["timetable"].get("days", [])
                    periods = data["timetable"].get("periods", [])
                    if not (0 <= day_index < len(days) and 0 <= period_index < len(periods)):
                        raise ValueError
